"""
ICL-Enhanced Vision Transformer (ViT-Base)
Strategy: Pure Prototypical Network (ProtoNet) for Few-Shot Classification using ViT-Base Features
"""
...
class ICL_ProtoNet(nn.Module):
    """
    Pure Prototypical Network for Few-Shot Classification with ViT-Base features.
    """
    
    def __init__(self, model_id: str = "google/vit-base-patch16-224",
                 num_classes: int = 2, feature_dim: int = 768): # ViT-Base feature dim is 768
        super().__init__()
        ...
        print(f"Loading ViT-Base model: {model_id}")
    ...
    def extract_features(self, images: list):
        """Extract ViT vision features using the [CLS] token output."""
        
        # Get the current device of the ViT backbone
        ...
        with torch.no_grad():
            ...
            # Get vision features (output is last hidden state)
            outputs = self.vit(image_inputs)
            
            # Extract features from the [CLS] token (the first token)
...
def train(model, train_loader, optimizer, criterion, device, epochs):
    ...
    # Ensure ViT backbone remains frozen/in eval mode
    model.vit.eval() 
...
def main():
    """Run Pure ProtoNet enhanced ViT-Base evaluation"""
    ...
    # ViT-Base feature dimension
    feature_dim = 768 
    
    print(f"\nInitializing ICL-enhanced ViT-Base ProtoNet (Feature Dim: {feature_dim})...")
    ...
    print("\n" + "="*60)
    print("ICL-ENHANCED: ViT-Base with Pure Prototypical Network")
    ...
    print_metrics(metrics, title="ICL-ViT-Base ProtoNet Results")
    ...
    save_results(
        ...
        os.path.join(results_dir, 'vit_protonet_results.json')
    )
    ...
    plot_confusion_matrix(
        cm,
        class_names,
        save_path=os.path.join(results_dir, 'vit_protonet_confusion_matrix.png')
    )
