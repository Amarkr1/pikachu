"""
ICL-Enhanced SigLIP (Pure Prototypical Network)
Strategy: Pure ProtoNet for Few-Shot Classification using SigLIP2 Features
"""
...
class ICL_ProtoNet(nn.Module):
    """
    Pure Prototypical Network for Few-Shot Classification with SigLIP2 features.
    """
    
    def __init__(self, model_id: str = "google/siglip2-base-patch16-224",
                 num_classes: int = 2, feature_dim: int = 768):
        super().__init__()
        ...
        print(f"Loading SigLIP2 model: {model_id}")
    ...
    def extract_features(self, images: list):
        """Extract SigLIP vision features (used by forward and train)"""
        
        # Get the current device of the SigLIP backbone
        model_device = next(self.siglip.parameters()).device 
        ...
        with torch.no_grad():
            ...
            # Get image features
            image_outputs = self.siglip.get_image_features(**image_inputs)
    ...
    def forward(self, query_images: list, support_images: list, support_labels: torch.Tensor):
        ...
        for class_idx in range(self.num_classes):
            ...
            if mask.any():
                ...
                # Handle empty prototype (though unlikely with proper sampling)
...
def train(model, train_loader, optimizer, criterion, device, epochs):
    ...
    # Ensure SigLIP backbone remains frozen/in eval mode
    model.siglip.eval() 
    ...
    for epoch in range(epochs):
        ...
        # Only the log_temperature parameter is in train mode (implicitly done by optimizer)
...
def main():
    """Run Pure ProtoNet enhanced SigLIP evaluation"""
    ...
    print("\nInitializing ICL-enhanced SigLIP ProtoNet...")
    ...
    print("\n" + "="*60)
    print("ICL-ENHANCED: SigLIP with Pure Prototypical Network")
    ...
    print_metrics(metrics, title="ICL-SigLIP ProtoNet Results")
    ...
    save_results(
        ...
        os.path.join(results_dir, 'siglip_protonet_results.json')
    )
    ...
    plot_confusion_matrix(
        cm,
        class_names,
        save_path=os.path.join(results_dir, 'siglip_protonet_confusion_matrix.png')
    )
